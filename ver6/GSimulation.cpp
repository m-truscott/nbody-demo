...
#pragma omp simd
      for (j = 0; j < n; j++) {
        for (int i = ii; i < ii + tileSize; i++) {
          ...
          acc_xtile[i - ii] += dx * G * particles->mass[j] * distanceInv *
                               distanceInv * distanceInv; // 6flops
          acc_ytile[i - ii] += dy * G * particles->mass[j] * distanceInv *
                               distanceInv * distanceInv; // 6flops
          acc_ztile[i - ii] += dz * G * particles->mass[j] * distanceInv *
                               distanceInv * distanceInv; // 6flops
        }
      }
#pragma omp simd
